...
def un_pickle_labeled_df():
    with open('../data/nmf_labeled_df.pkl', 'rb') as fp:
...
def one_user_to_pd(nmf_labeled_df, one_user_predictions):
    ...
    print(merged.info())
    merged = merged.sort_values(by='prediction', ascending=False)
    return merged
...
def prep_columns(df):
    df = df[['Board Game Rank','game_id','bgg_game','Description','Playing Time','Min Players', 'Max Players', 'Best Num Players', 'avg_rating', 'Avg Weight', 'nmf', 'Game', 'prediction']]
    df.columns = ['BGG Rank','game_id','bgg_game','Description','Playing Time','Min Players', 'Max Players', 'Best Num Players', 'Avg Rating', 'Complexity', 'Topic', 'Game', 'Prediction']
    return df

#once function rules them all
def for_flask(user_id, best_num_player, min_time, max_time):
    nmf_labeled_df = un_pickle_labeled_df()
    print("Unpickled labeled df")
    ugr_df, ugr_rdd = mongo_to_rdd_df()
    print("loaded df and rdd from mongo")
    ...
    if 5 in best_num_player:
        ...
        one_user_df = one_user_df.reset_index()
    print("filtered on user choices")
    #now we are ready to redistribute the top 8 based on topics
    one_user_df = redistribute(one_user_df)
    print("redistributed recommendations based on topics")
    one_user_df = one_user_df.round({'BGG Rank': 0, 'Complexity': 1 })
